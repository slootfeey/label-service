...
class LabelGenerator {
  constructor() {
    ...
    this.skuTextFontSize = 8;  // Reduced from 10 to 8
    this.kidslandFontSize = 7;
    this.padding = 4;
  }

  calculateEAN13Checksum(barcode) {
    ...
    return barcode.substring(0, 12) + checksum;
  }
  ...
  async createStickersPagePdf(orderData) {
    ...
    const finalBarcodeBarsWidth = this.barcodeTargetHeight;
    const finalBarcodeBarsHeight = this.barcodeTargetWidth;
    ...
    const numberTextY = -this.barcodeTargetWidth;
    const numberTextWidth = this.barcodeTargetWidth;
